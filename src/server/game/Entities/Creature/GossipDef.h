...
class TC_GAME_API InteractionData
{
    ...
    struct VendorTag;
    using VendorData = TaggedId<VendorTag>;
...
public:
    ...
    TrainerData const* GetTrainer() const { return std::holds_alternative<TrainerData>(_data) ? &std::get<TrainerData>(_data) : nullptr; }

    VendorData* GetVendor() { return std::holds_alternative<VendorData>(_data) ? &std::get<VendorData>(_data) : nullptr; }
    VendorData const* GetVendor() const { return std::holds_alternative<VendorData>(_data) ? &std::get<VendorData>(_data) : nullptr; }
...
private:
    ...
    std::variant<std::monostate, TrainerData, PlayerChoiceData, VendorData> _data;
};
